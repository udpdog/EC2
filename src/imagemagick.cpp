...
std::wstring quote(const std::wstring& v){
 ...
 for(wchar_t c:v){if(c==L'\\')++n;else if(c==L'\"'){r.append(n*2+1,L'\\');r+=c;n=0;}else{r.append(n,L'\\');n=0;r+=c;}}
 r.append(n*2,L'\\');return r+L'\"';
}
bool search(const wchar_t* name,std::wstring& result){
 DWORD n=SearchPathW(nullptr,name,nullptr,0,nullptr,nullptr);if(!n)return false;
 std::vector<wchar_t>b(n+1);DWORD w=SearchPathW(nullptr,name,nullptr,(DWORD)b.size(),b.data(),nullptr);
 if(!w||w>=b.size())return false;result.assign(b.data(),w);return true;
}
DWORD run(const std::wstring& exe,const std::wstring& args){
 ...
 CloseHandle(pi.hThread);WaitForSingleObject(pi.hProcess,INFINITE);DWORD code=1;GetExitCodeProcess(pi.hProcess,&code);CloseHandle(pi.hProcess);return code;
}
}
bool findImageMagick(std::wstring& exe){
 if(search(L"magick.exe",exe))return true;
 for(const wchar_t* var:{L"ProgramFiles",L"ProgramW6432"}){DWORD n=GetEnvironmentVariableW(var,nullptr,0);if(!n)continue;
  std::vector<wchar_t>b(n);if(!GetEnvironmentVariableW(var,b.data(),n))continue;std::error_code e;
  for(const auto& x:fs::directory_iterator(fs::path(b.data()),e)){if(e||!x.is_directory(e))continue;
   if(normalizeFormat(x.path().filename().wstring()).rfind(L"imagemagick",0))continue;
   fs::path p=x.path()/L"magick.exe";if(fs::is_regular_file(p,e)){exe=p.wstring();return true;}}
 }return false;
}
